...
def run(cfg, train_gnn, test_gnn, train_lm, pretrain_lm=None, test_lm=None):
    ...
    per_epoch_times = []
    total_times = []
    LM_PATH = f"/home/xiaoxin/TAG/.cache/{cfg.dataset}_LM_{cfg.logfile}.pt"
    for run in range(cfg.train.runs):
        set_seed(seeds[run])
...
def run_v3(cfg, train_gnn, test_gnn, train_lm, pretrain_lm=None, test_lm=None):
    ...
    for run in range(cfg.train.runs):
        ...
        for stage in range(1, cfg.train.stages+1):
            ...
            start = time.time()
            for epoch in range(1):
                lm_loss = train_lm(lm, dataloader, z, data, optimizer_lm, split_masks, evaluator, cfg.device, LM_PATH)
